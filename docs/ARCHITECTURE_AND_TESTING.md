...
O sistema dispõe de **20 temas institucionais completos**, definidos em `data/themes.py` e gerados para `static/css/color-themes.css`. Cada tema possui exatamente cinco cores de identidade: primária/sidebar, hover, acento, fundo e superfície. O **Preto Fosco Administrativo** (`paleta-01`) é o tema padrão para usuários sem preferência registrada. Entre as opções obrigatórias estão o tema neutro administrativo, o **Azul Marinho Registral** e o **Vinho Cartorial**; as demais famílias foram escolhidas para manter baixa saturação, conforto visual e diferenciação cromática.
